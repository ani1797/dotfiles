# layout-precommit.sh - Pre-commit hooks integration for direnv
# Auto-installs pre-commit hooks when entering a directory with .pre-commit-config.yaml
...
# 2. Install hooks if .pre-commit-config.yaml exists and hooks aren't installed
layout_precommit() {
